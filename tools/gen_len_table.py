...
"""
Usage:
  python tools/gen_len_table.py examples/bench1
  python tools/gen_len_table.py examples
  python tools/gen_len_table.py examples/bench1/bench_blocks.json
"""
...
MEM_SIZE_MAP = {
    ...
    # short forms some generators emit:
...
def compute_role_widths(op_in: str, ins: Dict[str, Any], dbinfo: Optional[OpDbInfo]) -> Tuple[int,int,int,int]:
    ...
    # scalar FP single/double: container context is XMM=128
    ...
    # movsx/movzx/movsxd/setcc: scalarize to 8/16/32/64 with 0 container
    if op.startswith("movzx"):
    ...
    # pure vector ops: data width = container width (128/256/512)
    if cont_w in (128,256,512):
        # (vmovd/vmovq are scalar-in-xmm; still treat as 128 container)
        if op == "vmovq": return 64, 64, 128, addr_w
        if op == "vmovd": return 32, 32, 128, addr_w
        return elem_w, cont_w, cont_w, addr_w

    # fallback scalar widths
...
def make_len_pkg(blocks: List[Dict[str, Any]],
    ...
    for i, (b, st, ff_norm) in enumerate(zip(blocks, stages, norm_ff_lists)):
        need = max(0, st - 1)
        if len(ff_norm) != need:
            print(f"[WARN] case {i}: stage={st}, ff={len(ff_norm)} (len={len(b.get('instructions', []) or [])})")
    ...
    if debug_rows_list is not None:
        with open(debug_path, "w", newline="") as f:
            ...
            w.writerow(["block_idx","uop_idx","opcode",
                        "elem_w","data_w","cont_w","addr_w","raw_operands"])
            w.writerows(debug_rows_list)

    print(f"[OK] {bench_dir.name}: N_BLOCKS={len(blocks)} -> {sv_out.name}")

def main():
    ap = argparse.ArgumentParser(description="Generate len_table_pkg.sv (SIMD-aware)")
    ...
    if src.is_dir() and not is_bench_dir(src):
        ...
        for b in benches:
            run_one_bench(b, None, op_db, args.debug_ew)
    else:
        if src.is_file():
